...
public interface BatchJpaRepository extends JpaRepository<Batch, Long> {
   ...
   @Query(value = "SELECT inDays FROM batches e WHERE calendar = ?1", nativeQuery = true)
   ...
   @Query(value = "SELECT * FROM batches WHERE calendar = ?1 AND date = ?2 AND date IS NOT NULL", nativeQuery = true)
   Set<Batch> findExceptionBatchesByCalendarIdAndDateNotNull(Long calendarId, Date date);

   @Query(value = "SELECT * FROM batches WHERE calendar = ?1 AND inDays LIKE %?2% AND date IS NULL", nativeQuery = true)
